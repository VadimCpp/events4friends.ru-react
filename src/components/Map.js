...
class Map extends React.Component {
  ...
  addPins = async (events) => {
    ...
    for (const event of events) {
      ...
      this.setState(state => {
        const { id, description, location, creator: { email }, start: { dateTime } } = event;
        const pins = state.pins.slice();
        pins.push({ id, description, location, email, dateTime, geolocation: { lng: result[0].x, lat: result[0].y }});
